            ...
            .then(function (modules) {
                for (var modname in modules) {
                    ...
                    for (var j=0;j<module.records.length; j++) {
                        ...
                        if (entry.label.search(regexp) >= 0) res.push({type: 'Record Class', name:entry.name, label: entry.label})
                    }
                    for (var j=0;j<module.reports.length; j++) {
                        var entry = module.reports[j];
                        if (entry.label.search(regexp) >= 0) res.push({type: 'Report', name:entry.name, label: entry.label})
                    }
                }
                return res;
            })
