...
Compiler.prototype = {
  ...
  prettyIndent: function (offset, newline) {
    ...
    if (this.parentIndents) this.buf.push("buf.push.apply(buf, __indent);");
  },
  ...
  visit: function (node) {
    ...
    if (debug) {
      this.buf.push('__jade.unshift({ lineno: ' + node.line + ', filename: ' + (node.filename ? JSON.stringify(node.filename) : '__jade[0].filename') + ' });');
    }
    ...
    if (mixin.call) {
      ...
      if (block || attrs.length) {
        ...
        if (attrs.length) {
          ...
          if (val.inherits) {
            this.buf.push('attributes: merge({' + val.buf + '}, attributes), escaped: merge(' + val.escaped + ', escaped, true)');
          } else {
  ...
  visitAttributes: function (attrs) {
    var val = this.attrs(attrs);
    if (val.inherits) {
      this.buf.push("buf.push(attrs(merge({ " + val.buf + " }, attributes), merge(" + val.escaped + ", escaped, true)));");
...
function isConstant(val) {
  ...
  if (/^ *("([^"\\]*(\\.[^"\\]*)*)"|'([^'\\]*(\\.[^'\\]*)*)'|true|false|null|undefined) *$/i.test(val)) return true;
  ...
  // Check arrays
  var matches;
  if (matches = /^ *\[(.*)\] *$/.exec(val)) return matches[1].split(',').every(isConstant);
