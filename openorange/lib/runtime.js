...
router.post('/record/:method', function (req, res, next) {
    ...
    rec[params.method].apply(rec, params.params).then(function (response) {
        if (params.calltype == 'instance') {
...
router.get('/select', function (req, res, next) {
    ...
    recClass.select().then(function (records) {
        res.send({
...
router.post('/oo/rollback', function (req, res, next) {
    oo.rollback().then(function (result) {
        res.send({ ok: true, response: result });
    }).catch(function (err) {
        console.log(err);
        res.send({ ok: false, error: err });
...
router.post('/oo/commit', function (req, res, next) {
    oo.commit().then(function (result) {
        res.send({ ok: true, response: result });
    }).catch(function (err) {
        console.log(err);
        res.send({ ok: false, error: err });
...
router.post('/oo/begintransaction', function (req, res, next) {
    oo.beginTransaction().then(function (result) {
        res.send({ ok: true, response: result });
    }).catch(function (err) {
        console.log(err);
        res.send({ ok: false, error: err });
