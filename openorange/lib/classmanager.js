...
classmanager.createClassStructure = function loadClassStructure() {
    ...
    for (var i = 0; i < this.reversed_scriptdirs.length; i++) {
        var sd = this.reversed_scriptdirs[i];
        var sdstruct = { sd: sd };
        var lookupdirs = this.lookupdirs;
...
classmanager.getClass = function getClass(name, options) {
    ...
    for (var i = min_sd; i <= max_sd; i++) {
        var sd = this.reversed_scriptdirs[i];
        for (var j = 0; j < this.lookupdirs.length; j++) {
            var ld = this.lookupdirs[j];
...
classmanager.SuperClass = function SuperClass(description) {
    var superclass = classmanager.getParentClassFor(description.name, description.inherits, description.filename);
