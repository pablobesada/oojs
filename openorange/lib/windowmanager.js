...
WindowContainer.createMatrixComponent = function createMatrixComponent(json, cls, detailname, rownr, rowfield) {
    ...
    if (json.pastewindow) {
        ...
        var params = {self: self, detailname: detailname, field: rowfield, rownr: rownr, json: json, editor: editors};
        pasteWindowComponent.click(self.openPasteWindow.bind(params));
    ...
    var res = recordClass.select().fetch()
        .then(function(records) {
            ...
            for (var j=0;j<records.length;j++) {
                ...
                var pasteparams = {self: self, detailname: params.detailname, rownr: params.rownr, field: field, fieldjson: fieldjson, pastewindow: pw, pastewindowelement: pwelement, editor: params.editor, record: rec};
...
WindowContainer.recordSelectedInPasteWindow = function recordSelectedInPasteWindow() {
    ...
    if (params.detailname == null) {
        ...
        var bind_params = {self: self, json: params.fieldjson, detailname: params.detailname, rowfield: params.field};   //self.window.getRecord().details(params.detailname)[params.rownr].fields(params.fieldjson.name)
